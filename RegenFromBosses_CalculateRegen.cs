		...
		public static void CalculateRegen()
		{
			if (Main.netMode!=NetmodeID.MultiplayerClient) //If playing singleplayer or hosting a server
			{
				...
				if (modLoadedModdersToolkit) //If Modder's Toolkit is loaded...
				{
					...
					{
						Main.NewText("[RegenFromBosses] HP/S from Bosses: " + (tempSlainBosses) + "/" + (tempCountBosses) + ": " + //...how many bosses are slain and the total amount of bosses...
						((float)((tempSlainBosses*configLifeFromBosses)/tempCountBosses)/2)); //...the new regeneration per second from bosses...
					}
